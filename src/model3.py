...
def train_validate(model, train_loader, val_loader, optimizer, scheduler, epochs, patience=3):
    ...
    for epoch in range(epochs):
        ...
        if val_f1 > best_f1:
            ...
            save_model(model)
            print(f'최고 성능 모델 저장: model3_electra_state.pth, model3_classifier_state.pth')
        else:
            patience_counter += 1
...
def save_model(model):
    ...
    """
    torch.save(model.electra.state_dict(), './models/model3/model3_electra_state.pth')
    torch.save(model.classifier.state_dict(), './models/model3/model3_classifier_state.pth')
...
if __name__ == "__main__":
    # 설정
    CSV_DIRECTORY = './data/model3/'  # 실제 CSV 파일들이 저장된 디렉토리 경로로 변경할 것
    EPOCHS = 20
    BATCH_SIZE = 16
    PATIENCE = 4
    ...
    model.to(device)
    model.electra.load_state_dict(torch.load('./models/model2/model2_electra_state.pth', map_location=device), strict=False)
