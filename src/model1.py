...
def train_validate(model, train_loader, val_loader, optimizer, scheduler, epochs, patience=3):
    ...
    for epoch in range(epochs):
        ...
        if val_f1 > best_f1:
            ...
            save_model(model)
            print(f'최고 성능 모델 저장: model1_electra_state.pth, model1_classifier_state.pth')
        else:
            patience_counter += 1
...
def save_model(model):
    ...
    """
    torch.save(model.electra.state_dict(), './models/model1/model1_electra_state.pth')
    torch.save(model.classifier.state_dict(), './models/model1/model1_classifier_state.pth')
...
def load_model(model_class, num_labels=4):
    ...
    model.electra.load_state_dict(torch.load('./models/model1/model1_electra_state.pth', map_location=device), strict=False)
    model.classifier.load_state_dict(torch.load('./models/model1/model1_classifier_state.pth', map_location=device), strict=False)
    model.to(device)
    model.eval()
    print("모델 로드 완료: model1_electra_state.pth, model1_classifier_state.pth")
    return model
...
if __name__ == "__main__":
    # 설정
    CSV_DIRECTORY = './data/model1/'  # 실제 CSV 파일들이 저장된 디렉토리 경로로 변경할 것
    EPOCHS = 20
    BATCH_SIZE = 16
    PATIENCE = 4
    ...
    # 로드한 모델로 테스트 세트 평가
    loaded_test_accuracy, loaded_test_f1 = evaluate(loaded_model, test_loader)
